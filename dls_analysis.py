...
def plot_data(x, y, params, temp, theta):
    ...
    if not os.path.exists("plots"):
        os.makedirs("plots")
    fig.savefig(
        "plots/gr_" + str(temp) + "C_" + str(round(theta)) + "degrees.pdf"
    )
...
def q(theta, wavelength=wavelength, n=n):
    ...
    q = (4 * np.pi * n * np.sin(np.radians(theta) / 2)) / (
        wavelength * 10 ** (-9)
    )
    return q
...
def stokes_einstein(temp, D, D_err, viscosity=viscosity, temp_err=temp_err):
    ...
    r_err = (constants.k * temp_err) / (6 * np.pi * D * viscosity) + (
        constants.k * temp * D_err
    ) / (6 * np.pi * viscosity * D**2)
    r_err *= 10**9
